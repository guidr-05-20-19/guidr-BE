  ...
  dbTrips.get(req.params.id)
    .then(post => {
      ...
      if (post.length !== 0) {
        ...
        res.status(404).json({ message: `Failed to get Trip with specific ID: ${req.params.id} does not exist` });
      }
    })
    .catch(err => {
      res.status(500).json({ message: `The Trip with id: ${req.params.id} could not be retrieved.`, error: err });
    });
}
...
const deleteTrip = async (req, res) => {
  ...
  try {
    ...
    if (recordsDeleted > 0) {
      ...
      res.status(404).json({ message: `Failed to delete Trip, Trip does not exist` });
    }
  } catch (err) {
...
const updateTrip = (req, res) => {
  if (req.params.id === undefined || req.body.trip_type === undefined || req.body.location === undefined) {
    res.status(400).json({ errorMessage: "Trip type and location are for the Trip are required." });
    return;
  }
  dbTrips.update(req.params.id, req.body)
    .then(count => {
